...
}string build = "";bool mark(string append){build += append + "\n";writeContentToFile("E:/workspace/java/ava/data-test/tsdv/Sample_for_R1_copy111/0testdriver_execution.txt", build);return true;}unsigned short UnsignedShortTest(unsigned short x){mark("statement={");
	mark("line-in-function=2###offset=54###statement=return x;");return x;

mark("statement=}");}int main(){try{using namespace utils;Elements data = readContentFromFile("E:/workspace/java/ava/data-test/tsdv/Sample_for_R1_copy111/input.txt");unsigned short x=data.findValueByName<unsigned short>("x");UnsignedShortTest(x); }catch(exception& error){build="Phat hien loi runtime";}return 0;}
