...
static int MAX_RECURSIVE = 3; // by default

// The maximum iteration for loop
static int MAX_LOOP_ITERATIONS = 4; // by default
...
}string build = "";bool mark(string append){build += append + "\n";writeContentToFile("E:/workspace/java/ava/data-test/tsdv/Sample_for_R1_copy111111/0testdriver_execution.txt", build);return true;}signed long int SignedLongIntTest(signed long int x){mark("statement={");
	if (mark("line-in-function=3###offset=62###statement=x > 0###control-block=if") && (x > 0)) {
				mark("line-in-function=4###offset=72###statement=return 1;");return 1;
	}
	else {
				mark("line-in-function=6###offset=92###statement=return 0;");return 0;
	}

mark("statement=}");}int main(){try{using namespace utils;Elements data = readContentFromFile("E:/workspace/java/ava/data-test/tsdv/Sample_for_R1_copy111111/input.txt");signed long int x=data.findValueByName<signed long int>("x");SignedLongIntTest(x); }catch(exception& error){build="Phat hien loi runtime";}return 0;}
