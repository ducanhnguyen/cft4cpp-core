...
}string build = "";bool mark(string append){build += append + "\n";writeContentToFile("E:/workspace/java/ava/data-test/tsdv/Sample_for_R1_copy/0testdriver_execution.txt", build);return true;}long long LongLongTest(long long x){mark("statement={");
	if (mark("line-in-function=3###offset=45###statement=x > 0###control-block=if") && (x > 0)) {
				mark("line-in-function=4###offset=55###statement=return 1;");return 1;
	}
	else {
				mark("line-in-function=6###offset=75###statement=return 0;");return 0;
	}

mark("statement=}");}int main(){try{using namespace utils;Elements data = readContentFromFile("E:/workspace/java/ava/data-test/tsdv/Sample_for_R1_copy/input.txt");long long x=data.findValueByName<long long>("x");LongLongTest(x); }catch(exception& error){build="Phat hien loi runtime";}return 0;}
