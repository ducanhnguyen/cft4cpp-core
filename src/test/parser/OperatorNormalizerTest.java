...
public class OperatorNormalizerTest {
    ...
    public void test01() throws Exception {
        Paths.CURRENT_PROJECT.ORIGINAL_PROJECT_PATH = new File(
                Utils.normalizePath(Paths.CURRENT_PROJECT.ORIGINAL_PROJECT_PATH))
                .getAbsolutePath();
        ProjectParser parser = new ProjectParser(new File(
                Utils.normalizePath(Paths.SYMBOLIC_EXECUTION_TEST)));
        IFunctionNode function = (IFunctionNode) Search.searchNodes(
                parser.getRootTree(), new FunctionNodeCondition(),
                "basicTest17(int,int,int)").get(0);

        Paths.CURRENT_PROJECT.TYPE_OF_PROJECT = Utils
                .getTypeOfProject(Paths.CURRENT_PROJECT.ORIGINAL_PROJECT_PATH);
        /*
		 * Normalize function
		 */
        FunctionNormalizer fnNormalizer = function
                .normalizeFunctionToFindStaticTestcase();
        String newFunctionInStr = fnNormalizer.getNormalizedSourcecode();
        ICPPASTFunctionDefinition newAST = Utils.getFunctionsinAST(
                newFunctionInStr.toCharArray()).get(0);
