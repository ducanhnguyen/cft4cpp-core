...
public abstract class AbstractTestdataGeneration implements ITestdataGeneration {
	...
	private boolean updateGUI(ITestdataExecution dynamic, ChangedTokens changedTokens, IFunctionNode originalFunction,
		...
		/*
		 * The test path generated by executing function is belonged to the
		 * normalized function. Therefore, this test path is transformed into
		 * the original test path.
		 * 
		 * Ex: we have a normalized test path "sv.age>0 => return 0". After
		 * transforming to the original test path, the test path becomes
		 * "sv.getAge()>0"
		 */
		...
		/*
		 * Convert the test data generated from dynamic test data generation
		...
		/*
		 * Save the old coverage before adding new test path into the tested
		 * function report
		 */
		...
		if (!isExistTestpath)
			try {
				...
				/*
				 * If coverage is increased, it means that the latest test path
				 * is useful :D
				 * 
