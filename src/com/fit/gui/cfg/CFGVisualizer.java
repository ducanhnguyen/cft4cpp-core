...
public class CFGVisualizer extends DragScrollPane implements LightTabbedPane.EqualsConstruct, ComponentListener {
	...
	public CFGVisualizer(IFunctionNode fn, MouseListener nodeListener, int cfgType, int cfgLevel) {
		this.fn = fn;
		...
		if (!canvas.hasAdapter()) {
			...
			try {
				if (cfgType == CFGVisualizer.OVERVIEW_CFG)
					cfg = new OverviewCFGGeneration(fn, cfgLevel).getOverviewCFG();
				else if (cfgType == CFGVisualizer.LOGIC_CFG) {
					FunctionNormalizer norm = fn.getGeneralNormalizationFunction();

					IFunctionNode clone = (IFunctionNode) fn.clone();
					clone.setAST(norm.getNormalizedAST());
					cfg = new CFGGenerationforBranchvsStatementCoverage(clone).generateCFG();
				}
				...
				e1.printStackTrace();
				JOptionPane.showMessageDialog(CFGVisualizer.this, "Dont support to display graph for this function",
