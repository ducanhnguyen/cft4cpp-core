...
public class CFGNode extends Node<ICfgNode> {
	...
	public CFGNode(List<ICfgNode> stmList) {
		...
		for (ICfgNode stm : stmList) {
			...
			if (count <= MAX_LINE) {
				...
				// Ä�Ã£ cÃ³ node trÆ°á»›c Ä‘Ã³ trÃªn dÃ²ng, xem liá»‡u cÃ³ thÃªm node má»›i
				// Ä‘Æ°á»£c
				// khÃ´ng
				if (width > 0)
				...
				// KhÃ´ng thÃªm Ä‘Æ°á»£c, táº¡o dÃ²ng má»›i cho node
				if (width == 0)
					if (len <= MAX_WIDTH) {
						width = len;
	...
	public boolean isCondition() {
		return getElement() instanceof ConditionCfgNode;
	}

	/**
	 * Kiá»ƒm tra nÃºt cÃ³ lÃ  nÃºt Ä‘iá»�u kiá»‡n Ä‘Æ¡n giáº£n <br/>
	 * (1 cÃ¢u lá»‡nh duy nháº¥t táº¡i nhÃ¡nh, hoáº·c cÃ¢u lá»‡nh return) hay khÃ´ng
	 */
	...
	protected void paint(Graphics2D g, int x, int y, int width, int height) {
		...
		case MARK:
			...
			// CÃ¢u lá»‡nh cuá»‘i Ä‘Æ°á»£c tÃ´ Ä‘en
	...
	public void triggerMouseDrag(MouseEvent e) {
		...
		// Xá»­ lÃ½ nhÃ³m cÃ¡c node khi áº¥n vÃ o nÃºt Ä‘iá»�u khiá»ƒn
		...
		} // Xá»­ lÃ½ nhÃ³m cÃ¡c node thÃ´ng thÆ°á»�ng
		else {
			CFGNode next = (CFGNode) getRefer()[0];

			// Náº¿u node tiáº¿p theo cÅ©ng lÃ  node thÃ´ng thÆ°á»�ng, di chuyá»ƒn theo
