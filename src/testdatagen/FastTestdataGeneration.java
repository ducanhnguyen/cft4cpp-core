...
public class FastTestdataGeneration extends AbstractTestdataGeneration {
	...
	private void DART_IMPROVEMENT(IFunctionNode originalFunction) throws Exception {
		...
		// Ignore execution when there exists duplicated constraints; duplicate
		// test
		// data
				...
				while (iteration_in_one_depth < MAX_ITERATIONS_IN_ONE_DEPTH && currentBranchCoverage < 100f) {
					...
					if (testdataExecution != null) {
						...
						if (!cfgUpdater.isCompleteTestpath()) {
							...
							AbstractTestdataGeneration.testdata
									.add(new TestdataInReport(fullTestdata, testdataExecution.getEncodedTestpath(),
											false, tmp_currentStatementCoverage, tmp_currentBranchCoverage,
						...
						// CASE 2. No errors during test data execution
							...
							AbstractTestdataGeneration.testdata
									.add(new TestdataInReport(fullTestdata, testdataExecution.getEncodedTestpath(),
											true, tmp_currentStatementCoverage, tmp_currentBranchCoverage,
											tmp_currentBranchCoverage > tmp_previous_currentBranchCoverage ? true
													: false || tmp_currentStatementCoverage > tmp_previous_currentStatementCoverage
															? true : false,
											tmp_isGenerateRandomly, improvedTestdata));// MODIFIED
									...
									while (!isFoundSolution && canNegateCondition && se.getConstraints().size() >= 1) {
										...
										do {
											...
											if (((PathConstraints) se.getConstraints())
												...
												if ((negatedCfgNode.getFalseNode() != null
														&& !negatedCfgNode.isVisitedFalseBranch())
							...
							if (testdata.length() > 0) {
								if (!existingTestdata.contains(testdata)) {
									...
									if (testdataExecution != null) {
										...
										// uncomplete (there arises an error
										// when the
										// test data execute)
										if (!cfgUpdater.isCompleteTestpath()) {
											logger.info("The testpath is uncomplete! We still update CFG");
	 ...
	 * Step 2. Execute test data to get test path. If error occurs, back to Step
	 * 1, else Step 2 <br/>
	 * Step 3. Get constraints<br/>
	 * Step 4. Negate constraints (from the last constraint). If exist, back to
	 * Step 2, else back to step 1<br/>
	 * 
	 ...
	 * </pre>
	 * 
	 * The above function has only one argument and it has been configured.
	 * <br/>
	 ...
	 * Ex: "int test(int a){a=a>0?1:2;}"---normalize---->"int test(int a){if
	 * (a>0) a=1; else a=2;}".
	 * 
	 * @param originalFunction
	...
	public static int MAX_ITERATIONS_IN_ONE_DEPTH = 1; // default
